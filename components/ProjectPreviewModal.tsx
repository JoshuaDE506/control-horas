...
function normalizarModoAcceso(
  ...
  /*
   * IMPORTANTE:
   * visibilidad y modo_acceso son
   * propiedades independientes.
   *
   * Nunca se usa visibilidad como
   * fallback del modo de acceso.
   */
  const raw = String(
...
export default function ProjectPreviewModal({
    ...
    const run =
      async () => {
        ...
        try {
          const res =
            await fetch(
              `/api/proyectos/${proyecto.id}/preview`,
              {
                method:
                  'GET',
                credentials:
  ...
  const miembrosUI =
    previewData?.miembros ??
    miembros;

  const statsUI =
    previewData?.estadisticasTareas ??
    estadisticasTareas;
  ...
  const flags =
    useMemo(() => {
       ...
       * NO permite inferir si el usuario
       * actual es miembro.
       *
       * La autoridad es meta.esMiembro.
       */
      const esMiembro =
        metaUI?.esMiembro ??
        false;
      ...
      /*
       * Las tareas son contenido interno.
       * Que un proyecto sea visible o de
       * acceso público NO vuelve públicas
       * sus tareas.
       */
      const puedeVerTareas =
        metaUI?.puedeVerTareas ??
        esMiembro;

      const totalMiembros =
        typeof metaUI?.totalMiembros ===
        'number'
          ? metaUI.totalMiembros
          : miembrosUI.length;
  ...
  const confirmAndJoin =
    async () => {
      ...
      try {
        const res =
          await fetch(
            `/api/proyectos/${proyectoUI.id}/unirse`,
            {
              method:
                'POST',
              credentials:
        ...
        setActionOk(
          'Te has unido exitosamente al proyecto.'
        );
        ...
        const ruta =
          `/dashboard/proyectos/${proyectoUI.id}/tareas`;
  ...
  const handleSolicitar =
    async () => {
      ...
      try {
        const res =
          await fetch(
            `/api/proyectos/${proyectoUI.id}/solicitudes`,
            {
              method:
                'POST',
              credentials:
        ...
        setActionOk(
          'Solicitud enviada. Espera la aprobación del creador.'
        );
  ...
  const mostrarBotonUnirse =
    !esMiembroUI &&
    flags.canJoinDirect &&
    modoAcceso ===
      'publico';

  const mostrarBotonSolicitud =
    !esMiembroUI &&
    !requestSentNow &&
    flags.canRequestInvite &&
    modoAcceso ===
      ...
      >
        <div className="sticky top-0 z-10 border-b border-white/10 bg-slate-900/95 p-4 backdrop-blur-xl sm:p-6">
          ...
          <div className="flex items-start justify-between gap-3 sm:gap-4">
            ...
            <button
              ...
              className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-slate-800/50 text-gray-400 transition-all hover:bg-slate-700/50 hover:text-white disabled:opacity-60"
              aria-label="Cerrar"
            >
              <svg
                className="h-6 w-6"
        ...
        <div className="flex-1 space-y-4 overflow-y-auto p-4 sm:space-y-6 sm:p-6">
            ...
            modoAcceso !==
              ...
              <div className="rounded-xl border border-white/5 bg-slate-800/30 p-4">
                <p className="text-sm text-gray-400">
                  {modoAcceso ===
                  'privado'
                    ? 'Método de acceso: Privado. Solo miembros invitados pueden unirse al proyecto.'
              ...
              <div className="rounded-xl border border-white/5 bg-slate-800/30 p-4">
                ...
                <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
                  ...
                  <div className="rounded-lg border border-gray-500/30 bg-gray-500/10 p-3 text-center">
                    <div className="text-xl font-bold text-gray-300 sm:text-2xl">
                      {
                        statsUI.todo
                      }
                    </div>
                  ...
                  <div className="rounded-lg border border-blue-500/30 bg-blue-500/10 p-3 text-center">
                    ...
                    <div className="mt-1 text-[11px] text-blue-300 sm:text-xs">
                      En progreso
                    </div>
                  </div>
                  ...
                  <div className="rounded-lg border border-green-500/30 bg-green-500/10 p-3 text-center">
                    ...
                    <div className="mt-1 text-[11px] text-green-300 sm:text-xs">
                      Completadas
                    </div>
                  </div>
                </div>
          ...
          {!flags.puedeVerTareas && (
            <div className="rounded-xl border border-white/5 bg-slate-800/30 p-4">
              <p className="text-sm text-gray-400">
                Las tareas del proyecto solo están disponibles para sus miembros.
              </p>
            </div>
          )}
