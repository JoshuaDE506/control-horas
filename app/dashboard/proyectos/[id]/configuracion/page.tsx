...
type PermisoNivel = 'dueno' | 'admin' | 'todos';
...
function getDuration(start: string, end: string): { text: string; isError: boolean } | null {
  if (!start || !end) return null;
  const ms = new Date(end).getTime() - new Date(start).getTime();
  if (ms < 0) return { text: 'La fecha de fin es anterior al inicio.', isError: true };
  const days = Math.round(ms / 86400000);
  return { text: `Duración estimada: ${days} día(s) (${Math.floor(days / 7)} semanas)`, isError: false };
}

function permisoApiToUi(v?: string | null): PermisoNivel {
  const val = (v ?? '').toLowerCase().trim();

  if (val === 'owner') return 'dueno';
  if (val === 'owner_admin' || val === 'admin') return 'admin';
  if (val === 'all_members' || val === 'todos') return 'todos';

  return 'dueno';
}

function permisoUiToApi(v: PermisoNivel): 'owner' | 'owner_admin' | 'all_members' {
  switch (v) {
    case 'dueno':
      return 'owner';
    case 'admin':
...
const EMPTY_FORM: ProjectSettingsForm = {
  ...
  fechaInicio: '',
  fechaFin: '',
  permisoEdicion: 'dueno',
  permisoGestionTareas: 'dueno',
};
      ...
      <div className="mb-5 flex items-start gap-3">
        ...
        <div className="flex min-w-0 items-start gap-2.5">
          ...
          <div className="min-w-0">
            <h2 className="leading-tight text-white text-base sm:text-lg font-bold">{title}</h2>
...
const IconFlag = () => (
  <svg className="h-5 w-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 21V3m0 0l9 4 9-4v12l-9 4-9-4V3z" />
  </svg>
);
...
export default function ProjectSettingsView() {
  ...
  const [form, setForm] = useState<ProjectSettingsForm>(EMPTY_FORM);
  const [initialForm, setInitialForm] = useState<ProjectSettingsForm | null>(null);
  ...
  const [puedeEditarConfig, setPuedeEditarConfig] = useState(false);
  const [puedeEliminarProyecto, setPuedeEliminarProyecto] = useState(false);
  ...
  const update = (key: keyof ProjectSettingsForm, value: any) => {
    if (!canEdit) return;
  ...
  const handleModoAcceso = (v: string) => {
    ...
    setForm((prev) => ({
      ...prev,
      modoAcceso: modo,
      visibilidad: modo === 'publico' ? 'publico' : 'privado',
    }));
    setIsDirty(true);
    setSaved(false);
  };
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        ...
        const res = await fetch(`/api/proyectos/${proyectoId}/configuracion`, {
          method: 'GET',
          credentials: 'include',
          cache: 'no-store',
        });
        ...
        const p = (data?.proyecto ?? data?.data?.proyecto) as ProyectoApi;
        if (!p) throw new Error('Proyecto no encontrado');

        const meta = data?.meta ?? data?.data?.meta ?? {};
        const permsMeta = meta.permisosConfiguracion ?? {};

        const mapped: ProjectSettingsForm = {
          nombre: p.nombre ?? '',
          descripcion: p.descripcion ?? '',
          estado: (p.estado as EstadoProyecto) || 'activo',
          visibilidad: (p.visibilidad as 'privado' | 'publico') || 'privado',
          modoAcceso: (p.modo_acceso as ModoAcceso) || 'privado',
          prioridad: (p.prioridad as PrioridadProyecto) || 'media',
          fechaInicio: p.fecha_inicio ?? '',
          fechaFin: p.fecha_fin ?? '',
          permisoEdicion: permisoApiToUi(permsMeta.permisoEdicion),
          permisoGestionTareas: permisoApiToUi(permsMeta.permisoGestionTareas),
        };
        ...
        const puedeEditar = Boolean(meta.puedeEditarProyecto);
        const puedeEliminar = Boolean(meta.puedeEliminarProyecto);

        setPuedeEditarConfig(puedeEditar);
        setPuedeEliminarProyecto(puedeEliminar);

        if (!puedeEditar) setShowNoPermission(true);
      } catch (err: any) {
  ...
  const handleSave = async () => {
    ...
    try {
      const payload = {
        ...
        modo_acceso: form.modoAcceso,
        prioridad: form.prioridad,
        fecha_inicio: form.fechaInicio || null,
        fecha_fin: form.fechaFin || null,
        permisoEdicion: permisoUiToApi(form.permisoEdicion),
        permisoGestionTareas: permisoUiToApi(form.permisoGestionTareas),
      };
      ...
      const p = (data?.proyecto ?? data?.data?.proyecto) as ProyectoApi | null;
      const meta = data?.meta ?? data?.data?.meta ?? {};
      const permsMeta = meta.permisosConfiguracion ?? {};
      ...
      setPuedeEditarConfig(Boolean(meta.puedeEditarProyecto));
      setPuedeEliminarProyecto(Boolean(meta.puedeEliminarProyecto));
  ...
  const handleDeleteProject = async () => {
    if (!proyectoId || !puedeEliminarProyecto || deleting) return;
  ...
  const duration = getDuration(form.fechaInicio, form.fechaFin);
  const nombreLeft = 80 - form.nombre.length;
  const descLeft = 400 - form.descripcion.length;
  const showSaveBar = canEdit && isDirty;

  const estadoColor = (v: EstadoProyecto) =>
    ({
  ...
  const prioridadColor = (v: PrioridadProyecto) =>
    ({
  ...
  const estadoOptions: ChipOption[] = [
    { value: 'activo', label: 'Activo', dot: '#4ade80' },
    { value: 'pausado', label: 'Pausado', dot: '#facc15' },
    { value: 'completado', label: 'Completado', dot: '#60a5fa' },
    { value: 'cancelado', label: 'Cancelado', dot: '#f87171' },
  ];
  ...
  const prioridadOptions: ChipOption[] = [
    ...
    { value: 'critica', label: 'Crítica', dot: '#f87171' },
  ];

  const modoAccesoOptions: ChipOption[] = [
    { value: 'privado', label: 'Privado' },
    { value: 'solicitud', label: 'Por solicitud' },
  ...
  const permisoOptions: ChipOption[] = [
    { value: 'dueno', label: 'Solo el dueño' },
    { value: 'admin', label: 'Admins' },
    { value: 'todos', label: 'Todos los miembros' },
  ];

  const ACCESS_HINTS: Record<ModoAcceso, string> = {
    privado: 'Solo invitados o miembros pueden ver y entrar al proyecto.',
    solicitud: 'Los usuarios deben solicitar acceso y esperar aprobación.',
    publico: 'Cualquiera puede unirse sin restricciones.',
  };

  const ESTADO_HINTS: Record<EstadoProyecto, string> = {
    activo: 'El proyecto está en marcha actualmente.',
    pausado: 'El proyecto está pausado temporalmente.',
    completado: 'El proyecto ha finalizado exitosamente.',
    cancelado: 'El proyecto fue cancelado.',
  };

  const PRIORIDAD_HINTS: Record<PrioridadProyecto, string> = {
    baja: 'Sin urgencia inmediata.',
    media: 'Prioridad estándar del proyecto.',
    alta: 'Requiere atención prioritaria.',
    critica: 'Máxima urgencia, atención inmediata.',
  };

  const PERM_EDICION_HINTS: Record<PermisoNivel, string> = {
    dueno: 'Solo el creador del proyecto puede editar esta configuración.',
    admin: 'El dueño y los admins del proyecto pueden editar.',
    todos: 'Cualquier miembro del proyecto puede editar la configuración.',
  };

  const PERM_TAREAS_HINTS: Record<PermisoNivel, string> = {
    dueno: 'Solo el creador del proyecto puede crear y editar tareas.',
    admin: 'Dueño y admins pueden crear y editar tareas.',
    todos: 'Cualquier miembro puede crear y editar tareas.',
  };
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-950 via-slate-900 to-slate-950 px-4 py-4 pb-32 sm:px-6 sm:py-6 lg:px-8 lg:py-10">
      ...
      <div className="mx-auto max-w-4xl space-y-5">
        <Section title="Identidad" subtitle="Nombre y descripción del proyecto" icon={<IconId />}>
          <Field
            label="Nombre del proyecto *"
            hint={canEdit ? `${nombreLeft} caracteres restantes` : undefined}
            hintColor={nombreLeft < 16 ? 'text-yellow-400' : 'text-gray-500'}
          >
            <input
              type="text"
              maxLength={80}
          ...
          <Field
            label="Descripción"
            hint={canEdit ? `${descLeft} caracteres restantes` : undefined}
            hintColor={descLeft < 50 ? 'text-yellow-400' : 'text-gray-500'}
          >
            <textarea
              maxLength={400}
              rows={3}
        ...
        <div className="grid grid-cols-1 gap-5 lg:grid-cols-2">
          <Section title="Estado" subtitle="Fase operativa actual" icon={<IconFlag />}>
            <div>
              <label className="mb-3 block text-sm font-medium text-gray-300">Estado del proyecto</label>
              <div className="grid grid-cols-1 gap-2 xs:grid-cols-2">
                {estadoOptions.map((opt) => {
                  const isActive = form.estado === (opt.value as EstadoProyecto);
                  return (
                    <button
                      key={opt.value}
                      type="button"
                      onClick={() => update('estado', opt.value as EstadoProyecto)}
                      disabled={!canEdit}
                      className={`flex items-center gap-2 rounded-xl border px-4 py-2.5 text-sm font-medium transition-all duration-200
                        ${
                          isActive
                            ? estadoColor(opt.value as EstadoProyecto)
                            : 'border-slate-700/50 bg-slate-800/50 text-gray-400 hover:bg-slate-700/50'
                        }
                        ${!canEdit ? 'cursor-not-allowed opacity-60' : ''}`}
                    >
                      <span className="h-2 w-2 flex-shrink-0 rounded-full" style={{ background: isActive ? opt.dot : '#4b5563' }} />
                      {opt.label}
                    </button>
                  );
                })}
              </div>
              <p className="mt-3 text-xs text-gray-500">{ESTADO_HINTS[form.estado]}</p>
            </div>
          </Section>

          <Section title="Prioridad" subtitle="Urgencia en la organización" icon={<IconChart />}>
            <div>
              <label className="mb-3 block text-sm font-medium text-gray-300">Nivel de prioridad</label>
              <div className="grid grid-cols-1 gap-2 xs:grid-cols-2">
                {prioridadOptions.map((opt) => {
                  const isActive = form.prioridad === (opt.value as PrioridadProyecto);
                  return (
                    <button
                      key={opt.value}
                      type="button"
                      onClick={() => update('prioridad', opt.value as PrioridadProyecto)}
                      disabled={!canEdit}
                      className={`flex items-center gap-2 rounded-xl border px-4 py-2.5 text-sm font-medium transition-all duration-200
                        ${
                          isActive
                            ? prioridadColor(opt.value as PrioridadProyecto)
                            : 'border-slate-700/50 bg-slate-800/50 text-gray-400 hover:bg-slate-700/50'
                        }
                        ${!canEdit ? 'cursor-not-allowed opacity-60' : ''}`}
                    >
              ...
              <p className="mt-3 text-xs text-gray-500">{PRIORIDAD_HINTS[form.prioridad]}</p>
            </div>
          </Section>
        </div>

        <Section title="Visibilidad y Acceso" subtitle="¿Quién puede ver y unirse al proyecto?" icon={<IconGlobe />}>
          <div className="grid grid-cols-1 gap-6 md:grid-cols-2">
            ...
            >
              <ChipGroup
                ...
                onChange={(v) => update('visibilidad', v as 'privado' | 'publico')}
                activeColorFn={gradientActive}
                disabled={!canEdit}
              />
            </Field>

            <Field label="Modo de acceso / membresía" hint={ACCESS_HINTS[form.modoAcceso]}>
              <ChipGroup
                options={modoAccesoOptions}
                value={form.modoAcceso}
                onChange={handleModoAcceso}
                activeColorFn={gradientActive}
                disabled={!canEdit}
              />
            </Field>
        ...
        <Section title="Fechas" subtitle="Plazos formales del proyecto" icon={<IconCalendar />}>
          ...
          {duration && (
            <p className={`mt-1 text-xs ${duration.isError ? 'text-red-400' : 'text-gray-500'}`}>
              {duration.isError ? '⚠ ' : ''}
              {duration.text}
            </p>
          )}
        ...
        <Section
          ...
          icon={<IconLock />}
        >
          <Field label="Edición del proyecto" hint={PERM_EDICION_HINTS[form.permisoEdicion]}>
            <ChipGroup
              options={permisoOptions}
              value={form.permisoEdicion}
              onChange={(v) => update('permisoEdicion', v as PermisoNivel)}
              activeColorFn={gradientActive}
              disabled={!canEdit}
            />
          </Field>
          ...
          <Field label="Creación y edición de tareas" hint={PERM_TAREAS_HINTS[form.permisoGestionTareas]}>
            <ChipGroup
              options={permisoOptions}
              value={form.permisoGestionTareas}
              onChange={(v) => update('permisoGestionTareas', v as PermisoNivel)}
              activeColorFn={gradientActive}
              disabled={!canEdit}
            />
          </Field>
      ...
      >
        <div className="mx-auto flex max-w-4xl flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          <p className="hidden text-sm text-gray-400 sm:block">
            Tienes cambios <span className="font-medium text-white">sin guardar</span>
          </p>
          ...
          <div className="ml-auto flex w-full flex-col gap-3 sm:w-auto sm:flex-row">
            ...
            <button
              type="button"
              onClick={() => {
                if (canEdit && !saving && isDirty) setShowSaveConfirm(true);
              }}
            ...
            >
              {saving && <div className="h-4 w-4 animate-spin rounded-full border-2 border-white/20 border-t-white" />}
      ...
      <div
        className={`fixed right-4 top-4 z-[200] flex max-w-[calc(100vw-2rem)] items-center gap-2 rounded-xl border border-green-500/30 bg-green-500/10 px-4 py-3 text-sm font-medium text-green-400 shadow-xl transition-all duration-300 sm:right-6 sm:top-6 sm:max-w-none sm:px-5 ${
          saved ? 'translate-y-0 opacity-100' : 'pointer-events-none -translate-y-2 opacity-0'
        }`}
      >
        <IconCheck />
        <span className="break-words">Configuración guardada correctamente</span>
      </div>

      <ConfirmModal
        isOpen={showSaveConfirm}
        title="Guardar configuración"
        message={`¿Deseas guardar los cambios en "${form.nombre || 'este proyecto'}"? La configuración se actualizará para todos los miembros.`}
      ...
      <ConfirmModal
        ...
        message={`No tienes permisos para modificar la configuración de "${form.nombre || 'este proyecto'}". Puedes ver los datos pero los campos están bloqueados. Contacta al dueño o un administrador si necesitas hacer cambios.`}
      ...
      <ConfirmModal
        ...
        message={`¿Seguro que deseas eliminar "${form.nombre || 'este proyecto'}"? Se borrará permanentemente junto con sus tareas y datos relacionados. Esta acción no se puede deshacer.`}
