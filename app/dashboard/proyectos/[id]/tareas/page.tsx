...
type PermisoGestionTareas = 'owner_admin' | 'todos_miembros';
...
function normalizarPermisoGestionTareas(
  valor: unknown
...
export default function TareasPage() {
  ...
  const [permisoGestionTareas, setPermisoGestionTareas] =
    useState<PermisoGestionTareas>('owner_admin');

  const [rolUsuario, setRolUsuario] =
    useState<RolUsuario>('miembro');
  ...
  const normalizarTareas = (raw: any[]): Tarea[] => {
    ...
    return raw.map((t: any) => ({
      ...
      tiempo_estimado_minutos:
        t?.tiempo_estimado_minutos == null
          ? null
          : Number(t.tiempo_estimado_minutos),
      max_participantes:
        t?.max_participantes == null
          ? 1
          : Number(t.max_participantes),
    }));
  };

  const extraerListaTareas = (payload: any): Tarea[] => {
    return normalizarTareas(
      payload?.tareas ??
        payload?.data ??
        []
    );
  ...
  const extraerTarea = (payload: any): Tarea | null => {
    ...
    return {
      ...
      tiempo_estimado_minutos:
        raw?.tiempo_estimado_minutos == null
          ? null
          : Number(raw.tiempo_estimado_minutos),
      max_participantes:
        raw?.max_participantes == null
          ? 1
          : Number(raw.max_participantes),
    };
  };
  ...
  const fetchAsignados = useCallback(
    async (
      ...
      const last =
        asignadosFetchedAtRef.current[tareaId];
      ...
      try {
        const res = await fetch(
          `/api/proyectos/${proyectoId}/tareas/${tareaId}/asignados`,
          {
            method: 'GET',
            credentials: 'include',
            cache: 'no-store',
  ...
  const refetchTareas = useCallback(async () => {
    try {
      ...
      const tareasData = await tareasRes
        .json()
        .catch(() => ({}));
  ...
  useEffect(() => {
    const cargarDatos = async () => {
      try {
        ...
        const permisoRaw =
          normalizarPermisoGestionTareas(
            proyectoData?.proyecto
        ...
        const puedeFromBackend =
          ...
          proyectoData?.data?.meta
            ?.puede_gestionar_tareas ??
          false;

        setPuedeGestionarTareas(
          Boolean(puedeFromBackend)
        );
        ...
        console.error(
          'Error cargando proyecto/tareas:',
          err
        );
  ...
  const crearTarea =
    useCallback(async () => {
      ...
      try {
        ...
        const dias =
          Number.isFinite(diasRaw) &&
          diasRaw > 0
            ? diasRaw
            : 0;

        const horas =
          Number.isFinite(horasRaw) &&
          horasRaw > 0
            ? horasRaw
            : 0;

        const minutos =
          Number.isFinite(minutosRaw) &&
          minutosRaw > 0
            ? Math.min(
                minutosRaw,
                59
  ...
  const handleSubmitTaskModal = () => {
    ...
    openConfirm({
      title: 'Crear tarea',
      message:
        '¿Confirmas la creación de esta nueva tarea?',
      type: 'success',
      confirmText:
        'Crear tarea',
      cancelText:
  ...
  const seleccionarTarea =
    async (tareaId: string) => {
      ...
      try {
        const res = await fetch(
          `/api/proyectos/${proyectoId}/tareas/${tareaId}/seleccionar`,
          {
            method: 'POST',
            credentials:
              'include',
  ...
  const comenzarTarea =
    async (tareaId: string) => {
      ...
      try {
        const res = await fetch(
          `/api/proyectos/${proyectoId}/tareas/${tareaId}/comenzar`,
          {
            method: 'POST',
            credentials:
              'include',
  ...
  const cancelarSeleccion =
    async (tareaId: string) => {
      ...
      try {
        const res = await fetch(
          `/api/proyectos/${proyectoId}/tareas/${tareaId}/cancelar`,
          {
            method: 'POST',
            credentials:
              'include',
  ...
  const reabrirTarea =
    async (tareaId: string) => {
      ...
      try {
        const res = await fetch(
          `/api/proyectos/${proyectoId}/tareas/${tareaId}/estado`,
          {
            method: 'PATCH',
  ...
  const confirmarSeleccion =
    (tareaId: string) => {
      openConfirm({
        ...
        message:
          '¿Confirmar selección de esta tarea?',
  ...
  const confirmarReabrir =
    (tareaId: string) => {
      openConfirm({
        ...
        message:
          '¿Confirmar reabrir esta tarea y devolverla a Por Hacer?',
        ...
        <div className="mb-6 flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
          <div className="min-w-0">
            ...
            <p className="mt-1 text-[11px] text-cyan-300">
              Rol detectado en este proyecto:{' '}
          ...
          <div className="flex w-full flex-col gap-2 sm:flex-row sm:flex-wrap lg:w-auto lg:justify-end">
            <button
              ...
              onClick={() =>
                router.push(
                  `/dashboard/proyectos/${proyectoId}/miembros`
                )
              }
            ...
            <button
              ...
              onClick={() =>
                router.push(
                  `/dashboard/proyectos/${proyectoId}/configuracion`
                )
              }
...
function ColumnaKanban({
  ...
  return (
    <div className="flex min-h-[280px] flex-col lg:min-h-0">
      <div
        className={`mb-3 rounded-lg border bg-gradient-to-r p-3 backdrop-blur-xl ${colorConfig[color]}`}
      >
        <h3 className="flex items-center justify-between text-sm font-semibold text-white">
          {titulo}

          <span
            className={`rounded-full px-2 py-0.5 text-xs font-bold ${badgeConfig[color]}`}
          >
            {tareas.length}
          </span>
        </h3>
...
function TareaCard({
  ...
  const irAConfiguracionesTarea =
    (
      ...
      router.push(
        `/dashboard/proyectos/${tarea.proyecto_id}/tareas/${tarea.id}/configuracion`
      );
    };
...
function ModalSeleccion({
  ...
  const tareaBloqueada =
    tarea.estado ===
      'completed' ||
    tarea.estado ===
      'review';

  const usuarioYaAsignado =
    asignados.some(
      (a) =>
        a.email ===
        userEmail
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 p-3 backdrop-blur-sm sm:p-4">
      <div className="max-h-[90vh] w-full max-w-md overflow-y-auto rounded-2xl border border-white/10 bg-slate-900 p-4 shadow-2xl sm:p-6">
        <h3 className="mb-2 break-words text-xl font-bold text-white">
          {tarea.titulo}
        </h3>
        ...
        'completed' ? (
          ...
          <div className="mb-4 rounded-lg border border-cyan-500/30 bg-cyan-500/10 p-3">
            <p className="text-sm text-cyan-300">
              Ya tienes seleccionada esta tarea.
            </p>
          </div>
        ) : cupoLleno ? (
