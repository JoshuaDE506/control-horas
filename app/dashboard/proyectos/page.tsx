...
type PermisoProyecto = 'owner_admin' | 'todos_miembros';
...
export default function ProyectosPage() {
  ...
  const [permisoEdicion, setPermisoEdicion] =
  ...
  const [usuariosEncontrados, setUsuariosEncontrados] =
    useState<UsuarioBusqueda[]>([]);
  const [miembrosSeleccionados, setMiembrosSeleccionados] =
    useState<UsuarioBusqueda[]>([]);
  ...
  const buscarUsuarios = async (query: string) => {
    ...
    try {
      ...
      const res = await fetch(
        `/api/user/buscar?q=${encodeURIComponent(termino)}`,
        {
          method: 'GET',
          credentials: 'include',
          cache: 'no-store',
  ...
  useEffect(() => {
    const timer = setTimeout(() => {
      void buscarUsuarios(busquedaMiembro);
    }, 300);
  ...
  useEffect(() => {
    if (vista === 'mios') {
      void fetchProyectos('creados');
    }

    if (vista === 'miembro') {
      void fetchProyectos('miembro');
    }
  }, [vista]);
  ...
  const agregarMiembro = (usuario: UsuarioBusqueda) => {
    if (
      !miembrosSeleccionados.some(
        (m) => String(m.id) === String(usuario.id)
      )
    ) {
  ...
  const removerMiembro = (usuarioId: string | number) => {
    setMiembrosSeleccionados((prev) =>
      prev.filter((m) => String(m.id) !== String(usuarioId))
    );
  };
                ...
                <div>
                  ...
                  <div className="relative">
                    <select
                      ...
                      onChange={(e) =>
                        setVisibilidad(
                          e.target.value as VisibilidadProyecto
                        )
                      }
                      className="w-full appearance-none rounded-xl border border-slate-700/50 bg-slate-900/40 px-4 py-3 text-white outline-none transition-all focus:border-purple-500 focus:ring-2 focus:ring-purple-500/20"
                    >
                      ...
                      <div>
                        ...
                        <div className="relative">
                          <select
                            ...
                            onChange={(e) =>
                              setPermisoEdicion(
                                e.target.value as PermisoProyecto
                              )
                            }
                            className="w-full appearance-none rounded-xl border border-slate-700/50 bg-slate-900/40 px-4 py-3 text-white outline-none transition-all focus:border-purple-500 focus:ring-2 focus:ring-purple-500/20"
                          >
                  ...
                  {fechaFin && (
                    <p className="mt-2 text-xs text-gray-500">
                      {(() => {
                        ...
                        if (dias < 0) {
                          return 'La fecha fin está en el pasado.';
                        }
        ...
        {proyectosVista.length === 0 ? (
          <div className="py-16 text-center sm:py-20">
            ...
            <h3 className="mb-2 text-xl font-semibold text-gray-300">
              {vista === 'mios'
                ...
                : 'No perteneces a ningún proyecto aún'}
            </h3>
          ...
          <div className="grid grid-cols-1 gap-4 sm:gap-6 md:grid-cols-2 xl:grid-cols-3">
            {proyectosVista.map((proyecto: any, index) => {
              const diasRestantes =
                calcularDiasRestantes(proyecto.fecha_fin);
                        ...
                        <div className="flex">
                          <button
                            ...
                            onClick={(e) => {
                              ...
                              router.push(
                                `/dashboard/proyectos/${idStr}/configuracion`
                              );
                            }}
