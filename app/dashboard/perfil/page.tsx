...
export default function PerfilPage() {
  ...
  async function cargarPerfil(mostrarCarga = true) {
    try {
      if (mostrarCarga) {
        setLoading(true);
      }
      ...
      const perfilNormalizado: Perfil = {
        ...
        activo: normalizarActivo(user.activo),
        creado_en: user.creado_en ?? null,

        proyectosCreados:
          Number(stats.proyectos_creados ?? 0) || 0,

        proyectosMiembro:
          Number(stats.proyectos_miembro ?? 0) || 0,

        tareasSeleccionadas:
          Number(stats.tareas_seleccionadas ?? 0) || 0,

        tareasEnProceso:
          Number(stats.tareas_en_proceso ?? 0) || 0,

        tareasCompletadas:
          Number(stats.tareas_completadas ?? 0) || 0,
      };
      ...
      setForm({
        ...
        pais:
          perfilNormalizado.pais === 'No definido'
            ? ''
            : perfilNormalizado.pais,
  ...
  function actualizarCampo<K extends keyof FormPerfil>(
    campo: K,
    valor: FormPerfil[K]
  ) {
    setForm((prev) => ({
  ...
  const pctCompletadas =
    totalTareas > 0
      ? Math.round(
          (perfil.tareasCompletadas / totalTareas) * 100
        )
      : 0;
      ...
      <div className="grid grid-cols-1 gap-4 xl:grid-cols-3">
        <div className="rounded-2xl border border-white/8 bg-slate-800/60 p-4">
          ...
          <div className="space-y-2">
            {tareaStats.map(
              ({ label, value, text, bar }) => {
                const pct =
                  totalTareas > 0
                    ? Math.round(
                        (value / totalTareas) * 100
                      )
                    : 0;
              ...
              ({
                ...
                return (
                  ...
                  >
                    ...
                    <p
                      className={`text-xl font-bold tabular-nums ${text}`}
                    >
                      {value}
                    </p>
