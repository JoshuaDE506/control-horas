...
export async function POST(
  ...
  try {
    ...
    // 2) Solo owner/admin pueden rechazar
    if (!esOwnerProyecto && !esAdminProyecto) {
      return NextResponse.json(
        {
          ok: false,
          error: 'Solo el owner o un admin pueden rechazar tareas',
        },
        { status: 403 }
      );
    }
    ...
    if (estadoActual !== 'review') {
      return NextResponse.json(
        {
          ok: false,
          error: 'Solo se puede rechazar una tarea que esté en review',
        },
        { status: 409 }
      );
    }
    ...
    await db.execute({
      sql: `
        ...
        WHERE id = ?
          AND proyecto_id = ?
      `,
      args: [comentario, now, String(tareaId), proyectoId],
    });
    ...
    console.error('POST /api/proyectos/[id]/tareas/[tareaId]/rechazar error:', error);
