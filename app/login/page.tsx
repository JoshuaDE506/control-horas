...
export default function LoginPage() {
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    if (!normalizedEmail || !password) { setError('Email y contraseña son obligatorios'); return; }
    setLoading(true);
    try {
      ...
      if (!response.ok || !data?.ok) {
        setError(response.status === 403
          ? (data?.message || 'Tu cuenta está pendiente de activación. Contacta a un administrador.')
      ...
      <div className="relative min-h-screen flex items-center justify-center px-4 py-8">
        <div className="w-full max-w-md">
          ...
          <div className="text-center mb-6 animate-slide-down">
            ...
            <h1 className="text-3xl sm:text-5xl font-bold text-white mb-2 tracking-tight">Bienvenido</h1>
            <p className="text-cyan-200 text-sm sm:text-base">Inicia sesión para continuar</p>
          </div>
            ...
            <div className="relative bg-white/10 backdrop-blur-2xl rounded-3xl shadow-2xl border border-white/20 p-6 sm:p-8 animate-slide-up">
              <form onSubmit={handleSubmit} className="space-y-5">
                {error && (
                  <div className="bg-red-500/20 border border-red-400/50 rounded-xl p-3 sm:p-4 animate-shake backdrop-blur-sm">
                    <div className="flex items-start gap-3">
                      ...
                      <span className="text-xs sm:text-sm text-red-200 font-medium leading-relaxed">{error}</span>
                    </div>
                  </div>
                )}
                ...
                <div className="space-y-2">
                  <label htmlFor="email" className="block text-sm font-semibold text-cyan-200">Correo Electrónico</label>
                  <div className="relative">
                    ...
                    <input
                      ...
                      disabled={loading}
                      className={`w-full pl-12 pr-4 py-3 bg-white/5 border-2 rounded-xl text-white text-sm placeholder-cyan-300/40 transition-all duration-300 focus:outline-none focus:bg-white/10 disabled:opacity-50 ${focusedField === 'email' ? 'border-cyan-400 shadow-lg shadow-cyan-500/30' : 'border-white/20 hover:border-white/30'}`}
                ...
                {/* Password */}
                <div className="space-y-2">
                  <label htmlFor="password" className="block text-sm font-semibold text-cyan-200">Contraseña</label>
                  <div className="relative">
                    <div className="absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none">
                      <svg className="h-5 w-5 text-cyan-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    <input
                      ...
                      disabled={loading}
                      className={`w-full pl-12 pr-12 py-3 bg-white/5 border-2 rounded-xl text-white text-sm placeholder-cyan-300/40 transition-all duration-300 focus:outline-none focus:bg-white/10 disabled:opacity-50 ${focusedField === 'password' ? 'border-cyan-400 shadow-lg shadow-cyan-500/30' : 'border-white/20 hover:border-white/30'}`}
                      placeholder="••••••••"
                    />
                ...
                <button type="submit" disabled={loading}
                  ...
                  <span className="relative flex items-center justify-center gap-2">
                    {loading ? (
                      ...
                      <><span>Iniciar Sesión</span><svg className="h-5 w-5 transition-transform group-hover:translate-x-1" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M13 7l5 5m0 0l-5 5m5-5H6" /></svg></>
                    )}
                  </span>
                </button>
              </form>
              ...
              <div className="relative my-4">
                ...
                <div className="relative flex justify-center text-sm">
                  <span className="px-4 bg-transparent text-cyan-200 font-medium">¿No tienes cuenta?</span>
                </div>
              </div>
